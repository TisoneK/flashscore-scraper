...
- **Identify by:** hostname `Baos-Mac-mini.local`, `$USER` = `bao`, repo at `/Users/bao/Code/flashscore-scraper`
- **OS:** macOS 15.7.7 (Darwin 24.6.0)
- **Runtimes:** node v24.17.0, Python 3.9.6 (system)
- **Package manager:** npm 11.13.0; pip via `python3 -m pip`
- **Verified commands:** git clone/commit/push (bootstrap session); project's own install/test commands not yet run here
- **Quirks:** system Python is 3.9 — check `pyproject.toml`/`requirements.txt` minimums before running; no project venv created yet
